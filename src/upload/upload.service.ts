...
export class UploadService {
  ...
  async uploadFile(user: Object, file: Express.Multer.File): Promise<string> {
    const summary: string = await this.processingService.summarizeFileContent(
      file,
    );
    const processedAudioResults: { filename: string; filepath: string } =
      this.processingService.convertSummmaryToAudio(summary, file.originalname);
    ...
    const s3Upload = await this.s3ClientService.s3Client.send(
